...
当前项目已完成阶段 9.2 的核心实现。审批恢复、记忆/上下文、Artifact、生产工具、多 Agent 后端基础、FastAPI、AG-UI/SSE、Conversation 多轮上下文、coder CLI、三种权限模式、Rich/NDJSON 事件流和持久化文件变更账本均已有实际实现。下一步依次完成 9.2.1 正确性收口、9.2.2 CLI 产品化和 9.3 确定性 software Team，再进入阶段 10 Web MVP；OpenTelemetry 目前只有局部 Span，完整可观测性、存储保留、高级 Git 和独立回滚 Run 属于阶段 11。
...
| `kernel/tool_bridge.py` | 已实现 | 从 ToolRegistry 构造 PydanticAI Tool Schema，同时强制所有实际调用经过 ToolExecutor；使用正常阶段内保持不变的预算指令，并根据剩余请求、工具和 Token 在预算允许时为最终回答保留最多三次输出尝试。 |
| `kernel/run_service.py` | 已实现 | 编排 Run 创建、状态变化、模型/只读工具循环、作用域记忆、Artifact 上下文、文本流事件、硬时长限制，以及完成、失败和取消终态；最终输出重试耗尽会分类为 `model_output_invalid`。 |
| `kernel/handoff.py` | 已实现 | 以根工作流 Run 确定性执行 Coordinator → Coder → Reviewer → Finalizer，分配父预算、创建真实子 Run、记录委派和阶段检查点，并支持暂停后续跑。 |
...
当前 API 已提供 Conversation 创建、列表、详情、有序消息读取和会话内新 Run。只有持久化 Conversation 会自动构建跨 Run 上下文；裸 Run 的 `conversation_id` 仍只用于分组和追踪。每轮保持独立预算、审批和能力租约。
...
| `cli/app.py` | 已实现 | 提供 `run/chat/inspect/replay/branch/resume/cancel/serve` 及 agents/tools/memory/changes 子命令，支持 Agent/权限模式、Rich 流式事件、Diff 审批、离线 Fake、JSON、迁移提示和 Ctrl+C 语义。 |
...
`agentcell chat` 已实现：它在同一 Conversation 下为每次用户输入创建新 Run，并支持按 Conversation ID 跨进程继续；不会把已完成 Run 当作可恢复 Run，也不会把上一轮剩余预算或临时授权带入下一轮。
