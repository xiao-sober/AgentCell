...
当前项目已完成阶段 9.2、9.2.1、9.2.2、9.3 和 9.4。应用组合根注册 transport-neutral `TaskRoutingService`，CLI `run/chat` 与 FastAPI `/task-routes`、`/tasks`、Conversation turn 共用同一 DTO、Policy 和执行服务。auto chat 的普通问答由确定性规则直接进入无工具 Assistant；工作区任务的明确意图使用确定性路由，歧义意图通过 PydanticAI 结构化输出进行有界分类。权威 `task-router` root 记录路由 Usage、版本化事件、decision hash、检查点、child、审批和终态，并实时接收 child 的公开文本、当前 Usage 和有界工具活动投影；投影保留来源 sequence、Agent、工具与安全参数摘要，以 `accounted_usage` 去重且不复制工具输出。`fixed/auto` Conversation 保存固定身份或 RoutingPolicy 版本、持久化模型绑定，并把有界历史传给每轮实际执行 Agent。
...
| `display.py` | 已实现 | 定义 CLI/Web 可复用的 RunDisplayState、纯事件 RunDisplayProjector、ToolDisplayCatalog、活动聚合、候选回答晋升、预算摘要和展示层凭据脱敏。不得依赖 Rich、Typer、FastAPI 或 React。 |
...
| `kernel/identity.py` | 已实现 | 定义版本化不可变执行身份，保存 AgentSpec/ModelSpec 快照及哈希，并在恢复、审批、委派和分支前阻止静默换模或换 Agent。 |
| `kernel/final_output.py` | 已实现 | Provider-neutral 最终输出防护，只拒绝以未解析 DSML、Function Call 或工具意图为主体的回答，并允许一次无工具预算内重试。 |
| `kernel/model_runtime.py` | 已实现 | 包装 PydanticAI Model，在每次模型请求前执行记忆注入、成对裁剪和 Artifact 压缩，并统一处理预算、Usage、完成事件和分类失败。 |
| `kernel/tool_bridge.py` | 已实现 | 从 ToolRegistry 构造 PydanticAI Tool Schema，同时强制所有实际调用经过 ToolExecutor；对模型可纠正且无副作用的租约参数或文件/目录类型错误最多触发一次预算内 ModelRetry，并在 FinalOutputGuard 重试时隐藏工具。 |
| `kernel/run_service.py` | 已实现 | 编排 Run 创建、不可变执行身份、状态变化、模型/工具循环、审批恢复、记忆与 Artifact 上下文、最终输出校验、稳定错误标识及完成、失败和取消终态。 |
| `kernel/handoff.py` | 已实现 | 以 Team 根 Run 确定性执行 Coordinator → Coder → Reviewer → Finalizer，持久化逐阶段模型/预算/职责/租约和证据；从工具事件与变更账本生成 Reviewer 证据，绿色零变更时移除其工具；审批决定、委派 Usage 与 root 对账位于同一应用服务边界，并在 Reviewer 门禁后才启动无工具 Finalizer。 |
...
| `budgets/models.py` | 已实现 | 定义不可变的 `Budget`、`Usage`、`BudgetRemaining` 和 `BudgetSnapshot` Pydantic 模型。Usage 同时保存输入、输出、缓存读取和缓存写入 Token；费用使用 `Decimal`，快照时间统一为 UTC。 |
| `budgets/tracker.py` | 已实现 | 实现内存预算记账并保留最近一次模型输入规模。Provider 请求、工具和子 Agent 在执行前预留；模型返回后记录实际 Token、缓存 Token 和费用。真实消耗即使导致超限也会保留，子预算及子树缓存用量必须回卷父 Run。 |
...
| `agents/registry.py` | 已实现 | 按稳定 ID 注册和查找 AgentSpec，独立保存 builtin/persisted/override 来源及 public/internal 可见性，并为运行时与产品列表提供不同投影。 |
| `agents/factory.py` | 已实现 | 使用 ProviderFactory 构造全新的 PydanticAI Agent，不保存 Run 级状态。 |
| `agents/builtins.py` | 已实现 | 提供默认只读/显式协作 coordinator，以及 coder、只读 reviewer、researcher、summarizer 和 finalizer。 |
| `agents/delegation.py` | 已实现 | 定义 Agent as Tool、委派持久化、结构化结果和程序化 Handoff 的严格领域契约。 |
| `agents/teams.py` | 已实现 | 定义版本化 TeamSpec/TeamRegistry、固定阶段依赖与职责，以及 root Budget 的请求/工具/Token 分权重分配和 CapabilityLease 的确定性最小化分配。 |
...
| `providers/deepseek.py` | 已实现 | 使用 PydanticAI DeepSeekProvider 构造官方平台 Model，映射 V4 `thinking.type` 与 `reasoning_effort`，处理思考模式 Function Calling 不接受 `tool_choice` 的协议差异，并在流式/非流式路径把 DeepSeek 缓存命中字段映射为统一 Usage。 |
| `providers/tool_names.py` | 已实现 | 将带命名空间的领域工具名映射为各 Provider 共同接受的 64 字符函数名子集。 |
...
| `tools/delegation.py` | 已实现 | 注册 `agent.delegate`，通过注入的 Kernel 协议创建受权限、预算、深度和恢复边界约束的子 Run。 |
...
| `memory/compaction.py` | 已实现 | 实现消息数量与估算 Token 双阈值裁剪、ToolCall/ToolReturn 原子保留、超大工具结果 Artifact 外置和请求前 ContextManager。 |
...
| `storage/tables.py` | 已实现 | 定义 Run/Event、Approval、Checkpoint、ToolExecution、AgentDelegation、Memory、Artifact ORM 表和 UTC 时间类型，保持领域模型与 ORM 分离。 |
| `storage/repositories.py` | 已实现 | 提供 Run/Event、审批/检查点、Agent 委派、Memory/Artifact Repository 和 SQLite 工具执行账本；包含原子事件 sequence、作用域 FTS5 候选查询及防重放。 |
...
| 文件或目录 | 状态 | 职责 |
| --- | --- | --- |
| `api/app.py`、`dependencies.py` | 已实现 | FastAPI 应用工厂、生命周期和显式应用依赖。 |
| `api/schemas.py`、`errors.py` | 已实现 | 稳定 DTO、统一 Problem Details 和领域错误状态码。 |
| `api/agui.py`、`sse.py` | 已实现 | 领域事件到官方 AG-UI 的纯映射及 sequence/offset 断线续传。 |
| `api/routes/` | 已实现 | Runs、Approvals、Changes、Agents、Memories、Providers、Tools、Health 和 Version 路由。 |

SSE/AG-UI 输出必须由领域事件映射而来，并支持按事件 sequence 断线续传。敏感配置不得通过 API 返回。

当前 API 已提供 Conversation 创建、列表、详情、有序消息读取和会话内新 Run。Conversation 同时持久化 `agent_id` 与 `model_ref`，每个新回合从该绑定生成执行 Agent；显式模型漂移返回冲突。只有持久化 Conversation 会自动构建跨 Run 上下文；裸 Run 的 `conversation_id` 仍只用于分组和追踪。每轮保持独立预算、审批和能力租约。
...
| `cli/app.py` | 已实现 | 保持薄 Typer 入口，提供 `run/chat/inspect/replay/branch/resume/cancel/serve` 及 agents/tools/memory 子命令，并调用独立 profile、审批、展示和 changes 模块。 |
| `cli/profile.py` | 已实现 | 将单 Agent 或显式 Team 与新旧 CLI 选项解析为 CliRunProfile/CliTeamProfile；Team 先建立 root 上限再分配子预算和子租约，任何越权组合明确拒绝。 |
| `cli/display.py` | 已实现 | 把中立 RunDisplayState 渲染为单一 Rich Live 双区或非 TTY 有界里程碑；审批和终态安全停止 Live，最终答案只打印一次。 |
| `cli/approvals.py` | 已实现 | 处理审批热键、pending、修改参数、同工具临时授权和非交互显式决定，并在读取输入前暂停 Live。 |
| `cli/changes.py` | 已实现 | 独立承载 changes list/show/diff/revert 命令和服务调用，保持主入口薄。 |
| `cli/common.py` | 已实现 | 共享 Console、数据库解析和一致的 CLI 错误边界。 |

CLI 直接调用共享应用服务，不通过 HTTP 请求本机 FastAPI；`run` 可用 `--max-requests`、`--max-tool-calls`、`--max-input-tokens` 和 `--max-total-tokens` 为单次真实任务显式扩大预算，`serve` 仅负责启动同一 FastAPI 应用工厂。默认累计 Token 预算为输入 200,000、输出 40,000、总计 240,000；完成行显示 Token 与缓存使用，JSON 在预算快照中返回相同字段。

`agentcell chat` 已实现：它在同一 Conversation 下为每次用户输入创建新 Run，并支持按 Conversation ID 跨进程继续；启动行显示持久化模型绑定，恢复时不会回退到新进程的默认模型。它不会把已完成 Run 当作可恢复 Run，也不会把上一轮剩余预算或临时授权带入下一轮。

阶段 9.2 已实现 `run/chat --agent coder` 的可用租约、三种权限模式、Diff 审批、每轮 Conversation ID、退出续聊命令和 Rich 事件流，并新增 ChangeSet/FileChange、before/after Artifact、可选 Git 工作区检查和安全反向变更。权限模式只控制审批，不会为 AgentSpec 增加工具或扩大 CapabilityLease；CLI 只展示公开领域事件，不读取或保存原始 ThinkingPart，Git 增强也不替代 AgentCell 自身的变更账本。
