...
2. `PairSafeTrimmer`：先按默认 100 条消息裁剪历史，扩展保留集合以确保 ToolCall 和 ToolReturn 不会只留下单边；
3. `ToolOutputCompactor`：超过 8 KiB 内联上限的 ToolReturn 写入 Artifact，消息中只保留摘要和引用；
4. `PairSafeTokenTrimmer`：按 Provider 中立的保守估算把上下文控制在约 32,000 Token，并把工具调用/结果作为不可拆分单元；
5. 内容发生裁剪或外置时记录 `context.compacted`，同时记录裁剪前后的消息数与估算 Token；召回记忆时记录 `memory.recalled`。

Token 估算按序列化消息的 UTF-8 字节数计算，并加入每条消息的协议开销。它不是厂商账单 Token 的替代品，而是用于在不同 Provider 之间提供一致、偏保守的请求前压缩边界。
...
`Conversation Memory` 仍是可按作用域检索的记忆投影；阶段 9.1 新增的 `messages` 才是一条会话的完整、有序消息权威记录。裸 Run 的 `conversation_id` 仍只用于分组和追踪，只有通过 ConversationService、Conversation API 或 `agentcell chat` 创建的新回合才会加载此前已完成 Run 的用户消息、模型回复和工具结果。

Conversation 多轮能力只继承经过工具对安全裁剪、Token 限制、Artifact 压缩和脱敏的会话内容，不继承上一轮剩余预算、临时审批或扩大的能力租约。ThinkingPart 和 Provider 私有元数据不会进入权威消息线程。
